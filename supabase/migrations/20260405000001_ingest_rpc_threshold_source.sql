-- FEAT-62: Update ingest_test_run RPC to accept and store threshold_source
-- from the evaluation JSONB payload. Backward compatible — old reporters
-- that omit threshold_source will insert NULL.
...
begin
  ...
  -- 3. Insert evaluations — now includes threshold_source
  with inserted as (
...
comment on function public.ingest_test_run(
  ...
  'evaluations insert + counter recomputation. Now includes threshold_source. '
