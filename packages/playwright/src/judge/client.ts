...
const DEFAULT_CONFIG: Required<Omit<JudgeConfig, 'openaiApiKey' | 'anthropicApiKey'>> = {
...
export class JudgeClient {
  ...
  constructor(config: JudgeConfig = {}) {
    ...
    this.openai = openaiKey
      ? new OpenAI({ apiKey: openaiKey, timeout: this.config.timeout, maxRetries: 0 })
      : null
  }
  ...
  private async callOpenAI(systemPrompt: string, userPrompt: string): Promise<JudgeResponse> {
